...
import com.common.zxing.light.Constant;
    ...
    class LightListenReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if ("lightchanged".equals(action)) {
                ...
                float lux=lightSensorManager.getLux();

                Camera camera=cameraManager.getCamera();
                Camera.Parameters parameters = camera.getParameters();

                Log.d( "亮度",String.valueOf( lux ) );
                Log.d("曝光度",String.valueOf( parameters.getExposureCompensation() ));
                if(Math.abs(lux+1)<0.001){
                    Toast.makeText( CaptureActivity.this,"传感器未打开" ,Toast.LENGTH_SHORT ).show();
                }else if(lux>300){
                    parameters.setExposureCompensation( -12 );
                    camera.setParameters(parameters);
                    lux=lightSensorManager.getLux();
                    Log.d( "调整后的亮度",String.valueOf( lux ) );
                    Log.d( "调整后的曝光度",String.valueOf( parameters.getExposureCompensation() ) );
                }
