...
public abstract class UPCEANReader extends OneDReader {
  ...
  static int[] findStartGuardPattern(BitArray row) throws NotFoundException {  //检测起始符
    boolean foundStart = false;
    int[] startRange = null;
    int nextStart = 0;
    int[] counters = new int[START_END_PATTERN.length]; //3
    while (!foundStart) {
      Arrays.fill(counters, 0, START_END_PATTERN.length, 0);
      startRange = findGuardPattern(row, nextStart, false, START_END_PATTERN, counters);
      int start = startRange[0];
      nextStart = startRange[1];
      // Make sure there is a quiet zone at least as big as the start pattern before the barcode. 确保存在至少与条形码前的起始模式一样大的静区。
   ...
   * @param rowOffset position to start search 开始搜索的位置
   * @param whiteFirst if true, indicates指示 that the pattern specifies white/black/white/...
   * pixel counts像素数, otherwise除此以外, it is interpreted as black/white/black/...
   * @param pattern pattern of counts of number of black and white pixels that are being
   * searched for as a pattern符号
