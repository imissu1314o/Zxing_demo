 ...
 * @author Sean Owen
 */
public class FinderPatternFinder2 {

    private static final int CENTER_QUORUM = 2;
    protected static final int MIN_SKIP = 3; // 1 pixel/module times 3 modules/center 1个像素/模块乘以3个模块/中心
    protected static final int MAX_MODULES = 57; // support up to version 10 for mobile clients 支持最高版本10的移动客户端
                    ...
                    if ((currentState & 1) == 0) { // Counting black pixels
                        if (currentState == 4) { // A winner?
                            if (foundPatternCross(stateCount)) { // Yes
                                boolean confirmed = handlePossibleCenter(stateCount, i, j, pureBarcode); //第i行 第j列
                                if (confirmed) {
                                    Log.d( "find mid","ok" );
...
//        Log.d( "stateCount",stateCount[0]+" "+stateCount[1]+" "+stateCount[2]+" "+stateCount[3]+" "+stateCount[4] );
    ...
    private float crossCheckVertical(int startI, int centerJ, int maxCount,
        ...
        int maxI = image.getHeight();
        int[] stateCount = getCrossCheckStateCount();
//        while(image.get( centerJ,startI )){
//            centerJ++;
//        }
//        centerJ--;
        // Start counting up from center
        int i = startI;  //垂直方向
        while (i >= 0 && image.get(centerJ, i)) { //startI上边到白块以前的黑块数量
            stateCount[2]++;
            i--;
        }
        if (i < 0) {
        ...
        while (i >= 0 && !image.get(centerJ, i) && stateCount[1] <= maxCount) { //统计白块数量
            stateCount[1]++;
            i--;
        }
        // If already too many modules in this state or ran off the edge:
        ...
        while (i >= 0 && image.get(centerJ, i) && stateCount[0] <= maxCount) { //统计外边框的黑块数量
            stateCount[0]++;
            i--;
        }
        if (stateCount[0] > maxCount) {
