...
public class FinderPatternFinder {
  ...
  public FinderPatternFinder(BitMatrix image, ResultPointCallback resultPointCallback,CaptureActivity activity) {
    this.image = image;
...
//        Log.d( "stateCount",stateCount[0]+" "+stateCount[1]+" "+stateCount[2]+" "+stateCount[3]+" "+stateCount[4] );
    ...
    FinderPattern[] patternInfo = selectBestPatterns();  //错误

    //
    ResultPoint.orderBestPatterns(patternInfo);  //排序
   ...
   */
  private boolean crossCheckDiagonal(int startI, int centerJ, int maxCount, int originalStateCountTotal) { //对角线检验
...
//      Log.d("1 and maxCount",stateCount[1]+" "+maxCount);
  ...
  private float crossCheckHorizontal(int startJ, int centerI, int maxCount,
    ...
    int minI=centerI,maxI=centerI;

    while(image.get( startJ,maxI )){
      maxI++;
    }
    maxI--;
    while(image.get( startJ,minI )){
      minI--;
    }
    minI++;
    Log.d( "I",minI+" "+maxI );
    ...
    for(int k=minI;k<=maxI;k++){
      ...
      while (j >= 0 && !image.get(j, k) && stateCount[1] <= maxCount) {
        stateCount[1]++;
        j--;
      }
      ...
      while (j >= 0 && image.get(j, k) && stateCount[0] <= maxCount) {
        stateCount[0]++;
        j--;
      }
      if (stateCount[0] > maxCount) {  //？
      ...
      while (j < maxJ && !image.get(j, k) && stateCount[3] < maxCount) {
        stateCount[3]++;
        j++;
      }
      ...
      while (j < maxJ && image.get(j, k) && stateCount[4] < maxCount) {
        stateCount[4]++;
        j++;
      }
      if (stateCount[4] >= maxCount) {
   ...
   * cross check with a vertical scan垂直线, and if successful, will, ah, cross-cross-check
   * with another horizontal scan水平线. This is needed primarily to locate the real horizontal
   * center of the pattern in cases of extreme skew.
   * And then we cross-cross-cross check with another diagonal scan对角线.</p>
   *
   ...
   * pattern center
   *
   * @param stateCount reading state module counts from horizontal scan  读取状态模块从水平扫描计数
...
//      centerJ = crossCheckHorizontal((int) centerJ, (int) centerI, stateCount[2], stateCountTotal);  //错误，因为原先（centerJ，centerI）指向白区，可以删除，也可以需要修改centerJ，让远点指向黑区
      if (!Float.isNaN(centerJ) &&
          (!pureBarcode || true)) {  //对角线检验 改？ //pureBarcode一般为false
  ...
  /**
   * @return true if we have found at least 3 finder patterns that have been detected检测
  ...
  private boolean haveMultiplyConfirmedCenters() {
    ...
    for (FinderPattern pattern : possibleCenters) {
      if (pattern.getCount() >= CENTER_QUORUM) {  //CENTER_QUORUM为2 每个定位符至少检测到两次
        confirmedCount++;
        totalModuleSize += pattern.getEstimatedModuleSize();
      }
    }
    if (confirmedCount < 1) {  //改？
      return false;
    }
    // OK, we have at least 3 confirmed centers, but, it's possible that one is a "false positive"
    // and that we need to keep looking. We detect this by asking if the estimated module sizes
    // vary too much. We arbitrarily say that when the total deviation from average exceeds
    // 5% of the total module size estimates, it's too much. 我们任意地说，当总平均偏差超过模块总尺寸估计值的5％时，它就太多了。
    float average = totalModuleSize / max;
    float totalDeviation = 0.0f;
    for (FinderPattern pattern : possibleCenters) {
      totalDeviation += Math.abs(pattern.getEstimatedModuleSize() - average);
    }
    return totalDeviation <= 0.05f * totalModuleSize;  //改？
  }
  ...
  private FinderPattern[] selectBestPatterns() throws NotFoundException {
    ...
    if (startSize < 3) {
//      doCameraZoom();
      Log.d("pattern","not find enough");
      // Couldn't find enough finder patterns
      throw NotFoundException.getNotFoundInstance();  //改？
//      return null;
    }else{
      Log.d("pattern","find enough");
    }
    ...
    if (possibleCenters.size() > 3) {
      // Throw away all but those first size candidate points we found. 除了我们找到的那些第一大小的候选点之外，扔掉所有。
